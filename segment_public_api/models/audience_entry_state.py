...
class AudienceEntryState(BaseModel):
    """
    AudienceEntryState
    """
    ...
    key: Key = Field(...)
    __properties = ["type", "audienceId", "transitions", "key"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('AUDIENCE_ENTRY'):
            raise ValueError("must be one of enum values ('AUDIENCE_ENTRY')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AudienceEntryState:
        """Create an instance of AudienceEntryState from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AudienceEntryState:
        """Create an instance of AudienceEntryState from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AudienceEntryState.parse_obj(obj)

        _obj = AudienceEntryState.parse_obj({
            "type": obj.get("type"),
