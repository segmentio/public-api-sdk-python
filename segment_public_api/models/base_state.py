...
class BaseState(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BaseState.construct()
        error_messages = []
        match = 0
        # validate data type: TransitionState
        if not isinstance(v, TransitionState):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TransitionState`")
        else:
            match += 1
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> BaseState:
    ...
    def from_json(cls, json_str: str) -> BaseState:
        ...
        # deserialize data into TransitionState
        try:
            instance.actual_instance = TransitionState.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ExitState
        try:
