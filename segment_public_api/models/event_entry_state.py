...
class EventEntryState(BaseModel):
    """
    EventEntryState
    """
    ...
    key: Key = Field(...)
    __properties = ["type", "condition", "transitions", "key"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('EVENT_ENTRY', 'AUDIENCE_ENTRY', 'DESTINATION', 'SIMPLE_DELAY', 'EVENT_SPLIT', 'CONDITION_SPLIT', 'RANDOM_SPLIT', 'EXIT', 'EXIT_RULE'):
            raise ValueError("must be one of enum values ('EVENT_ENTRY', 'AUDIENCE_ENTRY', 'DESTINATION', 'SIMPLE_DELAY', 'EVENT_SPLIT', 'CONDITION_SPLIT', 'RANDOM_SPLIT', 'EXIT', 'EXIT_RULE')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EventEntryState:
        """Create an instance of EventEntryState from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EventEntryState:
        """Create an instance of EventEntryState from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EventEntryState.parse_obj(obj)

        _obj = EventEntryState.parse_obj({
            "type": obj.get("type"),
