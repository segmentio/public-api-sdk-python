...
class GetDeliveryOverviewSourceMetricsBetaInput(BaseModel):
    """
    Input of the Delivery Overview Source steps.  # noqa: E501
    """
    source_id: StrictStr = Field(..., alias="sourceId", description="The sourceId for the Workspace.")
    destination_config_id: Optional[StrictStr] = Field(None, alias="destinationConfigId", description="The id tied to a Workspace Destination.")
    ...
    group_by: Optional[conlist(StrictStr)] = Field(None, alias="groupBy", description="A comma-delimited list of strings representing one or more dimensions to group the result by.  Valid options are: `eventName`, `eventType`, `discardReason`, and `appVersion`.")
    ...
    filter: Optional[DeliveryOverviewFilterBy] = None
    pagination: PaginationInput = Field(...)
    subscription_id: Optional[StrictStr] = Field(None, alias="subscriptionId", description="An optional filter for actions destinations, to filter by a specific action.")
    __properties = ["sourceId", "destinationConfigId", "startTime", "endTime", "groupBy", "granularity", "filter", "pagination", "subscriptionId"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetDeliveryOverviewSourceMetricsBetaInput:
        """Create an instance of GetDeliveryOverviewSourceMetricsBetaInput from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetDeliveryOverviewSourceMetricsBetaInput:
        """Create an instance of GetDeliveryOverviewSourceMetricsBetaInput from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetDeliveryOverviewSourceMetricsBetaInput.parse_obj(obj)

        _obj = GetDeliveryOverviewSourceMetricsBetaInput.parse_obj({
            ...
            "filter": DeliveryOverviewFilterBy.from_dict(obj.get("filter")) if obj.get("filter") is not None else None,
